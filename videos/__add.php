...
if($_POST['edited'] == 'true' && check_admin_referer( 'hdwplayer-nonce')) {
	...
	if($_POST['type'] == "vimeo"){
		if($_POST['thumb'] == '' || $_POST['preview'] == ''){
			$link = $_POST['video'];
			$link = str_replace('vimeo.com/', 'vimeo.com/api/v2/video/', $link) . '.php';
			$request = wp_remote_get($link);
			$output = wp_remote_retrieve_body( $request );
			$html_returned = unserialize($output);			
			if($_POST['thumb'] == ''){
	...
	if($_POST['type'] == "dailymotion"){
		if($_POST['thumb'] == '' || $_POST['preview'] == ''){
			...
			$request = wp_remote_get("https://api.dailymotion.com/video/$id?fields=thumbnail_medium_url,thumbnail_url");
			$output = wp_remote_retrieve_body( $request );
