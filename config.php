...
add_filter('query_vars','hdwplayer_plugin_add_trigger');
function hdwplayer_plugin_add_trigger($vars) {
	$vars[] = 'wid';
	$vars[] = 'view';
    $vars[] = 'vid';
    $vars[] = 'pid';
...
add_action('template_redirect', 'hdwplayer_plugin_trigger_check');
	function hdwplayer_plugin_trigger_check() {		
	...
	function configXML($id){
		...
		if($config->videoid){
			echo '<playListXml><![CDATA['.$siteurl.'/?vid='.$config->videoid.']]></playListXml>'.$br;
		} else {
			echo '<playListXml><![CDATA['.$siteurl.'/?pid='.$config->playlistid.$video_id.']]></playListXml>'.$br;
		}
	...
	function videoPlaylist($id){
		...
		echo '<video><![CDATA['.$item->video.']]></video>'.$br;
		if($item->hdvideo) {
			echo '<hd><![CDATA['.$item->hdvideo.']]></hd>'.$br;
		}
		echo '<streamer><![CDATA['.$item->streamer.']]></streamer>'.$br;
		if($item->dvr) {
			echo '<dvr>'.$item->dvr.'</dvr>'.$br;
		}
		echo '<thumb><![CDATA['.$item->thumb.']]></thumb>'.$br;
		if($item->token) {
			echo '<token>'.$item->token.'</token>'.$br;
		}
		echo '<preview><![CDATA['.$item->preview.']]></preview>'.$br;
		echo '<title><![CDATA['.$item->title.']]></title>'.$br;
		echo '</media>'.$br.$br;			
		echo '</playlist>'.$br;
		exit();
	}
	...
	if('flashvars' == $action)
	{
		...
		if ($detect->isMobile()) {
			switch ($results->type) {
				...
				case 'rtmp' :
					if($results->hls == ''){
						$results->hls = str_replace ( 'rtmp', 'http', $results->streamer ) . '/' . $results->video . '/playlist.m3u8';
					}
					if($detect->isDevice('android') && $results->rtsp != ''){
						$html5 = '<a href="'.$results->rtsp.'" style="position:relative; display: block;"><img style="position: absolute; margin: auto; top:0; bottom:0; right: 0; left:0" src="'.plugins_url() .'/'  . basename ( dirname ( __FILE__ ) ).'/assets/play.png"><img border="0" width="100%" height="100%" src="'.$results->preview.'">';
						$html5 .=  '</a>';
					}else{
						$html5 = '<video poster="' . $results->preview . '" onclick="this.play();" width="100%" height="100%" controls>';
						$html5 .= '<source src="' . $results->hls . '" />';
						$html5 .= '</video>';
					}
					break;
				default :
